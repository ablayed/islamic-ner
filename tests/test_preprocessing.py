...
def test_gazetteer_matches_bukhari_as_scholar(sample_gazetteer_dir: Path) -> None:
    ...
    scholar_matches = [match for match in matches if match["text"] == "\u0627\u0644\u0628\u062e\u0627\u0631\u064A"]
...
def test_gazetteer_longest_match_prefers_abu_hurayrah(sample_gazetteer_dir: Path) -> None:
    matcher = GazetteerMatcher(gazetteer_dir=str(sample_gazetteer_dir))
    ...
    assert any(
        match["text"] == "\u0623\u0628\u0648 \u0647\u0631\u064A\u0631\u0629" and match["entity_type"] == "SCHOLAR"
        for match in matches
    )
    assert not any(
...
def test_gazetteer_matches_tashkeel_variant(sample_gazetteer_dir: Path) -> None:
    ...
    normalized_canonical = ArabicNormalizer().normalize("\u0623\u0628\u0648 \u0647\u0631\u064A\u0631\u0629")
    ...
    matched = [
        match
        for match in matches
        if match["entity_type"] == "SCHOLAR" and match["canonical_name"] == normalized_canonical
    ]
    assert matched
